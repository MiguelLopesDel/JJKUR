...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageUraume;
...
public class UraumeScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption(Component.translatable("jujutsu.technique.uraume_5").getString()));
        options.add(new AltarOption(Component.translatable("jujutsu.technique.uraume_6").getString()));
        options.add(new AltarOption(Component.translatable("jujutsu.technique.uraume_8").getString()));
        options.add(new AltarOption(Component.translatable("jujutsu.technique.uraume_9").getString()));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(190, 210, 255, 150) : toRGB24(138, 180, 248, 150);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageUraume(this.hovered));
