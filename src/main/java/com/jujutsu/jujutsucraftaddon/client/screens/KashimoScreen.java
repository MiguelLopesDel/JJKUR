...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageKashimo;
...
public class KashimoScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption("Speed Blitz"));
        options.add(new AltarOption("Kaminari Strike"));
        options.add(new AltarOption("Lightning Barrage"));
        options.add(new AltarOption("Lightning Strike"));
        options.add(new AltarOption("Turn Up The Volume"));
        options.add(new AltarOption("Mythical Beast Amber"));
        options.add(new AltarOption("Domain Expansion"));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(255, 255, 255, 150) : toRGB24(255, 68, 218, 150);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageKashimo(this.hovered));
