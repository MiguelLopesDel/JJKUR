...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageMegumi;
...
public class MegumiScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption("Divine Dog Totality"));
        options.add(new AltarOption("Nue"));
        options.add(new AltarOption("Serpent"));
        options.add(new AltarOption("Toad"));
        options.add(new AltarOption("Max Elephant"));
        options.add(new AltarOption("Rabbit Escape"));
        options.add(new AltarOption("Round Deer"));
        options.add(new AltarOption("Piercing Ox"));
        options.add(new AltarOption("Tiger Funeral"));
        options.add(new AltarOption("Agito"));
        options.add(new AltarOption("MAHORAGA"));
        options.add(new AltarOption("Piercing Water"));
        options.add(new AltarOption("Giant Nue"));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(255, 255, 255, 150) : toRGB24(0, 0, 0, 150);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageMegumi(this.hovered));
            AltarMessageMegumi.pressAction(Minecraft.getInstance().player, this.hovered);
