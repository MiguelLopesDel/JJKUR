...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageHR;
...
public class HRScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption("Domain Breaker"));
        options.add(new AltarOption("Six Eyes Cut"));
        options.add(new AltarOption("Split Soul Cut"));
        options.add(new AltarOption("Ground Breaker"));
        options.add(new AltarOption("Healing"));
        return options;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageHR(this.hovered));
