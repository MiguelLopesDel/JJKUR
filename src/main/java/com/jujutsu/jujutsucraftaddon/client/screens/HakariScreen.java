...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageHakari;
...
public class HakariScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption("Ground Break"));
        options.add(new AltarOption("Doors"));
        options.add(new AltarOption("Kick Attack"));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(255, 255, 255, 150) : toRGB24(0, 100, 0, 150);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageHakari(this.hovered));
