...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageGojo;
...
public class GojoScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption(Component.translatable("jujutsu.technique.red").getString()));
        options.add(new AltarOption(Component.translatable("jujutsu.technique.purple").getString()));
        options.add(new AltarOption(Component.translatable("jujutsu.technique.blue").getString()));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(255, 255, 255, 150) : toRGB24(51, 153, 255, 150);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.hovered >= 0 && !this.selected) {
            this.selected = true;
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageGojo(this.hovered));
