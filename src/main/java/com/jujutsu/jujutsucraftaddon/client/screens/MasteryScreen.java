...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageMastery;
...
public class MasteryScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        options.add(new AltarOption("Extension Tech", 0, player));
        options.add(new AltarOption("RCT Mode", 1, player));
        options.add(new AltarOption("RCT BurnOut", 2, player));
        options.add(new AltarOption("RCT Output", 3, player));
        options.add(new AltarOption("Domain Mastery", 4, player));
        return options;
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (mc.player != null) {
            KeyMapping input = mc.options.keyUp;
            if (keyCode == mc.options.keyUp.getKey().getValue() ||
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < currentOptions.size(); i++) {
            ...
            int color = this.hovered == i ? toRGB24(255, 50, 50, 150) : toRGB24(0, 0, 0, 150);
    ...
    private static class AltarOption {
        ...
        void onSelect() {
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageMastery(this.index, this.playerUUID));
        }
    }
}
